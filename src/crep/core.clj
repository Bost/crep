...
(defn any-char [strn]
  (if (= "" strn)
    nil
    (list (first strn)
          ;; GraalVM can't use (.substring strn 1) nor (.substring strn 1 (count strn))
          ;; see https://github.com/graalvm/graaljs/issues/37
          (clojure.core/subs strn 1 (count strn)))))
...
(defn search
  "TODO make sure no param is missing using clojure.spec"
  [file ptrn cmt-str]
  (as-> (slurp file) $
    #_(subs $ 0 800)
    #_(block-parser $)
    (parse $)
    (filter (fn [b]
              ;; GraalVM can't use (.contains b ptrn)
              ;; see https://github.com/graalvm/graaljs/issues/37
              (clojure.string/includes? b ptrn)) $)))

(defn -main [& args]
  ;; (-> (read *in*)
  ;;     (get-in (mapv read-string args))
...
#_(-main
 "{:cmt-str \"#\" :files [\"/home/bost/dev/cheatsheet/cmds/linux.sh\"]}"
 "vim")
